#include "FEnergy.h"
//#include "oss2energy.h"
#include "ofOss2.h"
#include "../LocalSearch/LSDSCG.h"
...
double FEnergy::localopt(double **x0,int nAtom,int nO,double *fileAlpha,double acc,int maxStep,double &energy,double &rmsGrad){
        int i,j;
        OFOss2 bf(nAtom,nO,OFObjectiveFunction::MINIMIZE,nAtom*DGR,-10, 10);
        LSLocalSearch *ls;
//        bf  = new OFOss2(nAtom,nO,OFObjectiveFunction::MINIMIZE,nAtom*DGR,-8,8);
//        bf->ReadParam(fileAlpha);
	bf.ReadParam(fileAlpha);
        ls = new LSDFPMIN(&bf);
	//ls=new LSSimNelderMeadLS(bf);
	//ls = new LSBFGSLS(bf);
        ls->accuracy(acc);
        ls->nGenomEvaluations(maxStep);
        ls->stepLength(0.001);

        double x[SIZE_MAX*DGR];
        for(i=0;i<nAtom;i++)
                for(j=0;j<DGR;j++)
                        x[i*DGR+j]=x0[i][j];
        ls->genome(x);
        ...
        double oldenergy=bf.evaluate(x);

	ls->score(oldenergy);

        int iter=bf.evaluation();
        if(maxStep>0){
                ls->run();
                for(i=0;i<nAtom*DGR;i++) x[i]=ls->genome()[i];
        }
        iter=bf.evaluation()-iter;
        int iterConv=iter;

        energy=ls->score();
       	rmsGrad=bf.rmsGrad(x);
        delete ls;
//	delete bf;
        return oldenergy;
...
double FEnergy::rmsEnergy(double* alpha, vector<input>& inputs, bool print)
{
	double rmsEner=0;
    double ener_shift[3];
...
void FEnergy::evaluateAndPrint(double* x)
{
	double alpha[100];
...
int FEnergy::getNParam()
{
	return nparam;
}

void FEnergy::loadReference(const char* filename, input& ref_input)
{
...
void FEnergy::loadInputs(const char* filename)
{
	if (strlen(filename) == 0) return;
...
void FEnergy::setWeight(int pos, double w)
{
	weights[pos] = w;
}

double FEnergy::evaluate_( double const *x)
{
	neval++;
	double alpha[100];
...
void FEnergy::printBestAlpha(double* x, const char* fname)
{
...
void FEnergy::inv(double A[NMAX][NMAX], double Ainv[NMAX][NMAX], int N)
{
	int i, j, k;
	// for inversion
	double b[NMAX][NMAX];
...
double FEnergy::energy(double** x, int nAtom, int nO, double alpha[])
{
...
void FEnergy::energy_grad(double** x, int nAtom, int nO, double alpha[], double& energy, double grad[][3])
{
	int i, j, k, l, t;
...
double FEnergy::func(double* x, int nAtom, int nO, double alpha[])
{
	int i, j;
...
double FEnergy::dfunc(double* x, int nAtom, int nO, double alpha[], double* grad)
{
	double res;
	int i, j;
...
void FEnergy::lnsrch(double xold[], double fold, double g[], double p[], double x[],double *fret, double stpmax, int *check, int nAtom, int nO, double alpha[])
{
	int i;
...
int FEnergy::dfpmin(double p[], double gtol, int iterMax, double *fret,double *rmsGrad, int nAtom, int nO, double alpha[])
{

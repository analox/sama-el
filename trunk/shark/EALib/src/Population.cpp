...
*  Copyright (c) 1999-2000 Christian Igel
...
*        $RCSfile: Population.cpp,v $<BR>
*  
*    \par Changes:
*        $Log: Population.cpp,v $
*        Revision 2.6  2004/06/18 03:25:07  saviapbe
*        Some error messages of running of prepdoc.cpp were removed.
*
*        Revision 2.5  2004/06/17 23:09:00  saviapbe
*        An error in the INT's URL was corrected.
*
*        Revision 2.4  2004/06/17 22:53:19  saviapbe
*        The standard file header was for doxygen adapted.
*
*        Revision 2.3  2004/05/20 12:41:50  shark-admin
*        pvm_pkpop() and pvm_upkpop() modified
*  
*        Revision 2.2  2004/03/05 17:03:58  shark-admin
*        Cast warning in Population::pvm_upkpop() fixed (subPop, ascending, spinOnce)
*  
*        Revision 2.1  2004/01/30 15:25:04  shark-admin
				 ...
				 unsigned numElitists )
{
	...
	for( i = 0; i < size( ) && i < numElitists; i++ ) {
	    indvec[ i ]->selProb -= min( indvec[i]->selProb, 1./size( ) );
	    indvec[ i ]->numCopies++;
...
int Population::pvm_upkpop(){
  ...
  pvm_upkuint(s,1,1);
  if(this->size()!=*s){
    std::cerr << "EALib/Population.cpp: the population which has called pvm_upkpop() is of unexpected size!" << std::endl;
    std::cerr << "Please initialize this population prototypically." << std::endl;
    exit(-1);
  }
  delete s; 
...
void replaceUnconditional( Population& pop )
{
    ...
    for( unsigned i = min( numElements, pop.numElements ); i--; )

 ...
 *
 *  The class array serves as a basic data structure for building arrays
 *  of arbitrary types. It is implemented via a C++ template class
 *  array< type >. In principle an array can be viewed as an instance of
 *  an ordinary C-array, like type item[10][5][3]. However, this is a
 *  static instantiation of an array. The array template defined here is
 *  more flexible in that it is possible to dynamically resize or change
 *  the number of dimensions.  Furthermore, parameter passing by value and
 *  assignment works properly (i.e., the value is passed or assigned and
 *  not a pointer to the value) and the subscript operator [ ] may
 *  perform a range check at run-time (this is implemented by the exception
 *  mechanism of C++). <br>
 *  If the preprocessor directive "-DNDEBUG" is used, the range checks
 *  are disabled! <br>
 *  For convenience subscript operators for arrays of
 *  known dimensions are defined, for example:
 *  \code
 *          ...
 *      #include "Array/Array.h"   //  declarations
 *          ...
 *      Array< int > x( 10, 5 );   // define an array of integer values
 *      Array< int > y;            // define an array with unspecified size
 * 
 ...
 *      $RCSfile: Array.h,v $<BR>
 *      $Revision: 2.3 $<BR>
 *      $Date: 2005/05/23 10:02:08 $
 *
 *  \par Changes:
 *      $Log: Array.h,v $
 *      Revision 2.3  2005/05/23 10:02:08  christian_igel
 ...
 *
 *      Revision 2.2  2004/06/01 15:41:45  saviapbe
 *
 *      The bugs in the doxygen's documentation were removed.
 *
 *      Revision 2.1  2004/04/27 14:38:09  shark-admin
 *      bugfix in constructors (uninitialized memory leaks if empty array is created)
 *
 ...
 *      INI Administration
 *
 *      Revision 1.12  2002/05/16 13:18:34  rudi
...
// include this STL-header for the interface between vector<> and array<>
#include <vector>

// needed for type-safe I/O
#include <typeinfo>

// contains macros for runtime checking, may be disabled by the preprocessor
// directive "-DNDEBUG"
 ...
 *  the type-specific members like memory allocation, stream-I/O and the
 *  representation itself are defined in Array< T >, these members are
 *  accessible from ArrayBase via virtual functions, that is the interface
 *  is declared here. <br>
 *  Here you will mostly find methods for retrieving info
...
class ArrayBase
{
  public:  // interface of public members
    ...
    *
    *  \param  v the second array, which dimensions are compared to
    *            the one of the current array
    ...
    *
    *  The new dimensions of the current array are adopted from
    *  the dimensions of array \em v. <br>
    *  When the \em copy flag is set, then existing elements are
    *  copied (if possible), otherwise all existing elements are
    *  discarded.
    *
    ...
    *
    *  Using this method you can read and manipulate the dimension
    *  vector of the current array directly. <br>
  ...
  protected:  // interface of protected members (overloaded by array< T >)
    ...
    * 
    *  The default constructor should be never invoked directly since class
    *  arraybase contains pure virtual members.
    *
    *  \return none
    *
    *  \author  M. Kreutz
    ...
    *
    *  The copy constructor should be never invoked directly since class
    *  arraybase contains pure virtual members.
    *  Only the dimension vector ArrayBase::d is copied, the data vector 
    *  must be copied
    *  in the copy contructor of template class array< T >.
    *  This copy constructor is independent of the type of a particular
    *  template object of class array< T >.
    *
    ...
    *  \throw check_exception if the type of the exception is "type mismatch",
    ...
    *  \throw check_exception the type of the exception will be "I/O error" and
...
class Array : public ArrayBase
{
  ...
  public:
    ...
    *  \throw check_exception if the type of the exception is "size mismatch",
    ...
    *  \return the array with the new values
    *  \throw check_exception if the type of the exception will be 
    *         "size mismatch",
    ...
    *
    */            
    Array< T > remove_cols( const Array< unsigned > idx ) const
    {
        RANGE_CHECK( ndim( ) > 0 && idx.ndim( ) == 1 )

        unsigned i, j, k, l, ni, xi, zi;
        unsigned xlast = dim  ( ndim  ( ) - 1 );
        unsigned num   = nelem( ) / xlast;
        Array< T > z;
        ...
        for( ni = 1, i = 0; i < ix.nelem( ) - 1; ++i )
		{
            l = ix( k = i );
            for( j = i + 1; j < ix.nelem( ); ++j )
			{
                if( ix( j ) < l )
				{
                    l = ix( k = j );
        ...
        RANGE_CHECK( ix( ix.nelem( ) - 1 ) < dim( ndim( ) - 1 ) )
    ...
    *
    */            
    Array< T > remove_cols( unsigned i, unsigned j ) const
    {
    ...
    *
    */            
    Array< T > remove_cols( unsigned i, unsigned j, unsigned k ) const
    {
    ...
    Array< T >& transpose( )
    {
        ...
        switch( ndim( ) )
		{
            ...
            case 2 :
                ...
                break;
            default :
                unsigned size = nelem( ) / ( dim( 0 ) * dim( 1 ) );
                for( i = 0; i < dim( 0 ); ++i )
				{
                    for( j = 0; j < i; ++j )
					{
                        unsigned m = i * dim( 1 ) + j;
                        unsigned n = j * dim( 1 ) + i;
                        for( k = 0; k < size; ++k )
						{
                            std::swap( elem( m++ ), elem( n++ ) );
						}
                    }
				}
                break;
    ...
    *
    */                    
    Array< T > rotate_rows( int n ) const
    {
        SIZE_CHECK( ndim( ) > 0 )
            ...
            default :
                ...
                for( i = 0; i < num; ++i )
				{
                    for( j = 0; j < xlast; ++j )
					{
                        ...
                        newx.elem( i * xlast + l ) = elem( i * xlast + j );
                    }
				}
        }
    ...
    *  \sa ArrayBase::clone
    */
    ArrayBase* clone( ) const { return new Array< T >( *this ); }
    ...
    *
    */
    ArrayBase* empty( ) const { return new Array< T >( );       }
  ...
  protected:
        ...
        if( ne != _ne )
	{
                ...
                if( _ne )
		{
		    e = new T[ _ne ];
                ...
                for( j = ne < _ne ? ne : _ne; j--; e[ j ] = _e[ j ] );
                if( ne )
		{
		    delete[ ] _e;
		}
    ...
    *  \param is input stream from which the array content is read.
    *  \return none
    *  \throw check_exception the type of the exception is "type mismatch"

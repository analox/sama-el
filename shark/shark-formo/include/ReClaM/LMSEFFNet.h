...
public:
  ...
  LMSEFFNet(const unsigned in = 0, const unsigned out = 0, double min = 0., double max = 1.) : MSEFFNet(in, out) { tmin = min, tmax = max; };
  ...
  LMSEFFNet(const unsigned in, const unsigned out,
	    const Array<int>& cmat, double min = 0., double max = 1.) : MSEFFNet(in, out, cmat) { tmin = min, tmax = max; };
  ...
  LMSEFFNet(const unsigned in, const unsigned out,
	    const Array<int>& cmat, const Array<double>& wmat, double min = 0., double max = 1.) : MSEFFNet(in, out, cmat, wmat) { tmin = min, tmax = max; };
  
  //! Creates a MSE Bias feed-forward network by reading the necessary 
  //! information from a file named "filename".
  LMSEFFNet(const std::string &name, double min = 0., double max = 1.) : MSEFFNet(name) { tmin = min, tmax = max; };

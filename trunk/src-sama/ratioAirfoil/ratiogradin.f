C=====================================================================
C     Subroutine for finding 
C     the gradient of Cl(drag coefficient),  
C     the gradient of Cd(lift coefficient), 
C     the gradient of Cd/Cl,
C     and values of Cl, Cd, lift-to-drag ratio as well
...
C     ALPHIN(ND)
C     output:
C     CLOUT: lift coefficient
C     CDOUT: drag coefficient
C     GCL(ND): gradient of Cl
C     GCD(ND): gradient of Cd
C     GRAT(ND): a vector in the direction of the gradient of Cd/Cl 
C     RSDEU: residual of Euler equation at its last iteration
C     RSDADJCL: residual of the adjoint equation at its last iteration 
C               where Cl is used as the cost
C     RSDADJCD: residual of the adjoint equation at its last iteration
C               where Cd is used as the cost
C==========================================================================
      SUBROUTINE RATIOGRADIN(MACHIN,AOAIN,ALPHIN,CLOUT,CDOUT,GRAT,RSDEU,
     &           RSDADJCL,RSDADJCD)
      PARAMETER (ML=4,MI=161,MJ=91,MM=161)  
      PARAMETER (ND=24)	  
      REAL MACHIN,AOAIN
      DIMENSION ALPHIN(ND),GCL(ND),GCD(ND),GRAT(ND)
...
C     ---- variables,arrays for 2-d adjoint eq. only---------
      ...
      COMMON /SGM/SGM1,SGM2,THTA1,THTA2
...
C     --------- Calculate adjoint gradient for CL ----------
      SGM1=0.0
      SGM2=1.0
      THTA1=-SGM1*SIN(ALP)-SGM2*COS(ALP)
      THTA2=SGM1*COS(ALP)-SGM2*SIN(ALP)
      CALL ADJT2D
      RSDADJCL=RSDM
      ITLADJCL=ITL
	  CALL GRAD
...
C     --------- Calculate adjoint gradient for CD ----------
      SGM1=1.0
      SGM2=0.0
      THTA1=-SGM1*SIN(ALP)-SGM2*COS(ALP)
      THTA2=SGM1*COS(ALP)-SGM2*SIN(ALP)
...
C     ------- Calculate adjoint gradient for drag/lift ratio ----
      DO I=1,ND
         GRAT(I)=(CL*GCD(I)-CD*GCL(I))/CL**2
      END DO
      
C10   WRITE(*,20) CDOUT, CLOUT, RSDEU, RSDADJCL, RSDADJCD
C20   FORMAT(/3X,'CD = ',F8.6,/3X,'CL = '
C    1     ,F8.6,/3X,'EULER residu   = ',F12.10,/3X,
C    2     'Adjoint residus = ',F12.10,/1X,F12.10)
C     WRITE(*,*) 'End solvers'
C     WRITE(*,*) '-----------------------------------'
C     WRITE(*,*)
...
C     ------ the eigenvalue for which the failure occurs.
	     ...
	     ELSE
            WRITE(*,*) 'IERR=',IERR
            WRITE(*,*) 'havent obtained eigenvalue at No.', IERR
         END IF
...
C     ----------------------------------------------------
     ...
     7           +PSET(4,I,J)*DS2F(4,I,J)
      ...
      DO I=1,NI1
         ...
         BB=-DSTRBD(I)
         PPB=AA*PP(I,1)+BB*PP(I,2)
         DLI=DLI+2.*PPB*(THTA1*DXXISF(I)+THTA2*DYXISF(I))
      END DO
...
C     --- upper half surface ---------
      NDHF=ND/2
      DO I=1,201
         XI=FLOAT(I-1)/FLOAT(201-1)
         XC=0.+1.0*PLYNOM(0.,1.,0.01,0.02,XI)
         XSF(201+I-1)=XC
         YSF(201+I-1)=YUS(ALPHA,XC)
      END DO
C     --- lower half body surface --------
      DO I=1,201
         XI=FLOAT(I-1)/FLOAT(201-1)

 ...
 *      $RCSfile: CrossEntropyIndependent.h,v $<BR>
 *      $Revision: 2.1 $<BR>
 *      $Date: 2004/06/01 15:41:45 $
 *
 *  \par Changes:
 *      $Log: CrossEntropyIndependent.h,v $
 ...
 *      stable
 */
class CrossEntropyIndependent : virtual public ModelInterface {
  ...
  double derror(const Array<double> &in, const Array<double> &target, bool returnError=true) {
	...
	for(unsigned i = 0; i < dedw.nelem(); i++)
	  for(unsigned c = 0; c < output.nelem(); c++)
	    if((output(c)!=0) && (1.0-output(c)!=0))
	      dedw(i) -= (  (target(c) - output(c))/( (1. - output(c)) * output(c) )  ) * dmdw(c,i);
	    else if(output(c)!=target(c)) 
	      dedw(i) -= ( 2.0*target(c)-1.0 ) * maxDeriv * dmdw(c,i);
      } else {
	...
	for(unsigned pattern = 0; pattern < in.dim(0); ++pattern) {
	  ...
	  for(unsigned i = 0; i < dedw.nelem(); i++)
	    for(unsigned c = 0; c < output.nelem(); c++)
	      if((output(c)!=0) && (1.0-output(c)!=0))
		dedw(i) -=  (  (target(pattern,c) - output(c))/( (1. - output(c)) * output(c) )  ) * dmdw(c,i);
	      else if(output(c)!=target(pattern, c))
		// the sign changes due to the value of the target, maxDeriv is devided by reason of computational bounds
		dedw(i) -= ( 2.0*target(pattern,c)-1.0 ) * maxDeriv/in.dim(0) * dmdw(c, i);	
	}
      }
    else
      // one output neuron
	...
	for(unsigned i = 0; i < dedw.nelem(); i++)
	  if ((output(0)!=0) && (1.0-output(0)!=0))
	    dedw(i) -= (target(0)-output(0))/(output(0)*(1.0-output(0))) * dmdw(0, i);
	  else if (output(0)!=target(0))
	    dedw(i) -=( 2.0*target(0)-1.0 ) * maxDeriv * dmdw(0, i);
      } else {
	...
	for(unsigned pattern = 0; pattern < in.dim(0); ++pattern) {
	  ...
	  for(unsigned i = 0; i < dedw.nelem(); i++)
	    if ((output(0)!=0) && (1.0-output(0)!=0))
	      dedw(i) -= ( target(pattern,0)-output(0))/(output(0)*(1.0-output(0)) ) * dmdw(0, i);
	    else if (output(0)!=target(pattern,0))
	      // the sign changes due to the value of the target, maxDeriv is devided by reason of computational bounds
	      dedw(i) -=( 2.0*target(pattern,0)-1.0 ) * maxDeriv/in.dim(0) * dmdw(0, i);
	}
      }
    if (!returnError)
      ce = -1;

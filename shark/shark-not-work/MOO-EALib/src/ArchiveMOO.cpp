 ...
 *      $RCSfile: ArchiveMOO.cpp,v $<BR>
 *
 *  \par Changes:
 *      $Log: ArchiveMOO.cpp,v $
 *      Revision 2.6  2004/12/30 11:51:13  shark-admin
 *      saveArchiveGPT added: resulting file is readable for gnuplot
 *
 *      Revision 2.5  2004/11/15 15:37:06  shark-admin
 *      compatibility with vc++ ensured
 *
 *      Revision 2.4  2004/06/18 03:45:50  saviapbe
 *      Dummy linking to examples.
 *
 *      Revision 2.3  2004/06/18 01:56:34  saviapbe
 *      Was prepared for doxygen.
 *
...
void ArchiveMOO::addArchive( IndividualMOO& indmoo )
{
  if ( getMaxArchive( ) <= size( ) ){
    cout << getMaxArchive( ) << " " << size( ) << endl;
    cout << "Full capacity in TO-AM-050" << endl;
    exit( -1 );
  }
  else{
...
IndividualMOO& ArchiveMOO::readArchive( unsigned i )
{
  if ( i < size( ) ){
    ...
    cout << "Access an unused archive in TO-AM-051" << endl;
    exit( -1 );
  }
}
...
void ArchiveMOO::delArchive( unsigned i )
{
  if ( i < size( ) ){
    ...
    cout << "Access an unused archive in TO-AM-052" << endl;
    exit( -1 );
  }
}
...
void ArchiveMOO::delArchive( std::vector< unsigned >& v )
{
  for ( unsigned i = v.size( ); i--; ){
    if ( v[i] >= size( ) ){
      cout << "Access an unused archive in TO-AM-053" << endl;
      exit( -1 );
    }
    else{
      delArchive( v[i] );
...
void ArchiveMOO::delArchive( Array< unsigned >& a )
{
  for ( unsigned i = a.nelem( ); i--; ){
    if ( a(i) >= size( ) ){
      cout << "Access an unused archive in TO-AM-054" << endl;
      exit( -1 );
    }
    else{
      delArchive( a(i) );
...
{
  if ( i1 >= size( ) || i2 >= size( ) ){
    cout << "Access an unused archive in TO-AM-150" << endl;
    exit( -1 );
  }
  double distance = 0.0;
...
double ArchiveMOO::distanceOnFitness( IndividualMOO& im1 )
{
  ...
  double distance = 0.0;
  if ( No == 0 ){
    cout << "Archive is empty in TO-AM-151" << endl;
	 #ifdef _WIN32
...
unsigned ArchiveMOO::sharingWorst( double div )
{
  ...
  if ( size( ) == 0 ){
    cout << "Archive has no individual in TO-AM-154" << endl;
    exit( -1 );
  }

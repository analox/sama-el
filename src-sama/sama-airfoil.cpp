...
//
double airfoil( vector<double>& x );
...
int main( int argc, char** argv ) {
	...
	ifstream bFile("ratioAirfoil/minmax24.dat");
	for(int i = 0; i < dimension; i++) {
		bFile >> lowestBound[i] >> highestBound[i];
		cout << lowestBound[i] << " " << highestBound[i] << endl;
	}
	bFile.close();
	...
	while( mcount < maxCount )  {
		...
		else
		{
			...
			if (stagCount >= STAGMAX) {
				cout << "Restart population due to stagnation..." << endl;
				for(i=0; i<parent.size(); i++) {
                			dynamic_cast<ChromosomeT<double>&>(parent[i][0]).initialize(tempLoBound,tempHiBound);
...
double objectiveFunction( vector<double>& x ) {
	...
	// Get current time after evaluation 
	double currTime = (getTime() - startRunTime)/1000000.0;
	...
	ofstream bestVecFile("logs/bestVecFile.dat", ios::app);
        ...
        bestFile << mcount << "\t" << currBest << "\t" << currTime << endl;
        bestVecFile << mcount << "\t";
        for( i=0; i<dimension; i++ ) {

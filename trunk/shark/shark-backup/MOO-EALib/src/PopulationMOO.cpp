 ...
 *  Created :  2001-12-16
 *
 *  Description : Class PopulationMOO in EALib for MOO
 ...
 *      $RCSfile: PopulationMOO.cpp,v $<BR>
 *
 *  \par Changes:
 *      $Log: PopulationMOO.cpp,v $
 *      Revision 2.22  2006/04/30 17:00:50  christian_igel
 *      simplified
 *
 *      Revision 2.21  2006/04/19 11:09:04  glasmtbl
 *      CompareSortCrowdedTournamentFitness: >= replaced by >
 *
 *      Revision 2.17  2005/06/21 09:51:19  christian_igel
 *      SMeasure added
 *
 *      Revision 2.16  2005/06/10 13:04:43  glasmtbl
 *      *** empty log message ***
 *
 *      Revision 2.15  2005/01/31 09:37:59  shark-admin
 *      new default parameter 'unpenalized' added to crowdingDistance, some BubbleSorts exchanged
 *
 *      Revision 2.14  2005/01/17 11:04:50  shark-admin
 *      Bug in selectBinaryTournament(...) corrected
 *
 *      Revision 2.13  2004/12/30 13:09:27  shark-admin
 *      selectBinaryTounamentMOO(...)changed to selectBinaryTournamentMOO(...)
 *
 *      Revision 2.12  2004/12/27 16:03:32  shark-admin
 *      selectBinaryTournament corrected
 *
 *      Revision 2.11  2004/12/10 18:37:05  christian_igel
 *      New selection mechanisms added.
 *
 ...
 *
 *      Revision 2.8  2004/05/20 12:04:06  shark-admin
 *      pvm_pkpop(...) and pvm_upkpop(...) modified
 *
 *      Revision 2.7  2004/04/27 15:03:49  shark-admin
 *      constructors are bugfixed: addition of 'subPop = false;' due to memory leak caused by class EALib/Population destructors
 *      void nichedComparisonRank(...) with changed return type 'void'
 *
 *      Revision 2.6  2004/04/13 02:14:44  shark-admin
 *      Definitions added for 'Array<unsigned> nichedComparisonRank(...)', 'void selectCrowdedEPTournament(...)'.
 *
 *      Bug fixed in 'crowdedDistance( )': case handled if 'max-min=0' (nan)
 *
...
// ----------------------------------------------------------------------------
// TO-PM-210
int PopulationMOO::Dominate( unsigned i1, unsigned i2 )
{
  ...
  if ( (*this)[i1].getNoOfObj( ) != (*this)[i2].getNoOfObj( ) ){
    ...
    throw("wrong number of objectives");
  }
...
void PopulationMOO::selectElitists( PopulationMOO& offspring, unsigned numElitists )
{
  if ( numElitists ){
    ...
    for ( i = 0; i < size( ) && i < numElitists; i++ ){
      ...
      indvec[i]->setSelProb( dtemp - min( dtemp, 1.0/size( ) ) );
...
void PopulationMOO::NormalizeSelectProb( )
{
  ...
  if ( total == 0.0 ){
    ...
    throw("cannot compute selection probability");
  }
  for ( i = size( ); i--; ){
  ...
  for ( unsigned rank = 1; rank <= maxrank; rank++ )
...
#ifdef _WIN32
	(*this)[ global[ sort[ 0 ] ] ].setMOOShare( numeric_limits< double >::max( ) );
	(*this)[ global[ sort[ counter - 1 ] ] ].setMOOShare( numeric_limits< double >::max( ) );
#else
	...
	for ( i = 1; i < counter - 1; i++ ){
	  temp1 = (*this)[ global[ sort[ i ] ] ].getMOOShare( );
         #ifdef _WIN32
	  if ( temp1 != numeric_limits< double >::max( )  ) 
         #else
	    if ( temp1 != MAXDOUBLE )
         #endif
	      {
...
void PopulationMOO::SMeasure( bool UnpenalizedFitness )
{
  ...
  for ( unsigned rank = 1; rank <= maxrank; rank++ ) {
    ...
    unsigned obj = 0;
    // initialization of index vector
    for ( i = 0; i < size( ); i++ ) { sort[ i ] = i; assigned[ i ] = false; } 
    // sort
    bool changed;
    do {
      changed  = false;
    ...
    for(unsigned e = 1; e < counter - 1; e++) { // loop over all non-border elements
...
PopulationMOO& PopulationMOO::combinePopulationMOO( PopulationMOO& p1, PopulationMOO& p2 )
{
  if ( p1.checkData( p2 ) != 0 ){
    ...
    throw("cannot combine populations");
  }

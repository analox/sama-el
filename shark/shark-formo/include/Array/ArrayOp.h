 ...
 *      $RCSfile: ArrayOp.h,v $<BR>
 *      $Revision: 2.1 $<BR>
 *      $Date: 2005/10/11 13:58:29 $
 *
 *  \par Changes:
 *      $Log: ArrayOp.h,v $
 *      Revision 2.1  2005/10/11 13:58:29  christian_igel
 *      changed oder of functions in file
 *
 ...
 *      INI Administration
 *
 *      Revision 1.12  2002/09/17 08:35:02  rudi
 *      Added identifier "typename" in template definitions for gcc 3.x
 *
 *      Revision 1.11  2002/06/26 15:31:42  rudi
 *      Latest modifications of min- and max-methods were erroneous.
 *
 *      Revision 1.10  2002/05/16 13:18:35  rudi
 ...
 */
template < class T >
inline T min( const Array< T >& v, unsigned & ind,
 ...
 */
template < class T >
inline T max( const Array< T >& v, unsigned & ind,
              unsigned start = 0, unsigned end = 0)
{
    RANGE_CHECK( v.nelem( ) > 0 )
    RANGE_CHECK( v.nelem( ) > start)
 ...
 */
template < class T >
inline void minmax( const Array< T >& v, T& minVal, T& maxVal )
{
    if( v.nelem( ) > 0 ) {

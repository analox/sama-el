 ...
 *      $RCSfile: EarlyStopping.cpp,v $<BR>
 *      $Revision: 2.2 $<BR>
 *      $Date: 2005/08/03 12:00:15 $
 *
 *  \par Changes:
 *      $Log: EarlyStopping.cpp,v $
 *      Revision 2.2  2005/08/03 12:00:15  christian_igel
 ...
 *      INI Administration
 *
 *      Revision 1.3  2002/02/06 15:35:00  rudi
...
EarlyStopping::EarlyStopping(unsigned sl = 5) { 
  GLvalue     = 0;
#ifndef __SOLARIS__
  e_opt       = std::numeric_limits< double >::max( );
  e_va_old    = std::numeric_limits< double >::max( );
#else
  e_opt       = MAXDOUBLE;
  e_va_old    = MAXDOUBLE;
#endif

 ...
 *      $RCSfile: NoisyIOSamples.h,v $<BR>
 *
 *  \par Changes:
 *      $Log: NoisyIOSamples.h,v $
...
class NoisyIOSamples : public IOSamples< double >
{
  public:
    ...
    void setMissingProb( double p ) { coinToss.prob( p );  }
    void setSNR        ( double s ) { snr     = s;         }
    void setWindowSize ( double w ) { winSize = w;         }
    void setVariance   ( double v ) { gauss.variance( v ); }

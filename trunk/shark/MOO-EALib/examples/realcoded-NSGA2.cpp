...
void calcFitnessApproximate(IndividualMOO &i,
                        unsigned Task,
                        vector<double>& Min,
                        vector<double>& Max,
                        Array<double> & Basis,
...
void rankTempOffsprings( PopulationMOO &tempOffsprings, PopulationMOO &parents ) {
...
int main( int argc, char* argv[] )
{
	...
	PopulationMOO tempOffsprings( param.ArchiveSize, ChromosomeT< double >( param.Dimension ) );
	...
	//for ( t = 1; t < 81; ++t ) 
	{
		...
		tempOffsprings.selectBinaryTournamentMOO(parents);

		 recombine by crossing over two parents
		for ( i = 0; i < tempOffsprings.size( ); i += 2 )
			if ( Rng::coinToss( param.pc ) )
			dynamic_cast< ChromosomeT< double >& >(tempOffsprings[i][0]).SBX(dynamic_cast< ChromosomeT< double >& >(tempOffsprings[i+1][0]), lower, upper, param.nc,param.pbc);

		// mutate by flipping bits and evaluate objective function
		for ( i = 0; i < tempOffsprings.size( ); ++i )
		{
			// modify
			dynamic_cast< ChromosomeT< double >& >(tempOffsprings[i][0]).mutatePolynomial(param.MinInit, param.MaxInit, param.nm, param.pm);

			// evaluate objective function
			calcFitnessApproximate( tempOffsprings[ i ], param.Task, lower, upper, B, param.rpcon1, param.rpcon2 );
		}

		
		// choose which one to be exact evaluated
		// rankTempOffsprings based on number of solutions dominates it and crowding distance
		rankTempOffsprings( tempOffsprings, parents );	
		
		offsprings[ 0 ] = tempOffsprings[ 0 ];
		calcFitness( offsprings[ 0 ], param.Task, lower, upper, B, param.rpcon1, param.rpcon2 );

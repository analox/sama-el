 ...
 *
 *      Revision 1.4  2002/02/06 14:56:19  rudi
 ...
 */
  double derror( const Array< double >& input, const Array< double >& target, bool returnError=true ){
    ...
    double mse = gradientMSE( input, target, ModelInterface::dedw )/(double)outputDimension;
    dedw = dedw / (double)outputDimension;
    if (!returnError)
      mse = -1;
    return mse;
  }

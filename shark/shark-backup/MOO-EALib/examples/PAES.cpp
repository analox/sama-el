 ...
 *      $RCSfile: PAES.cpp,v $<BR>
 *
 *  \par Changes:
 *      $Log: PAES.cpp,v $
 *      Revision 2.7  2004/12/30 14:12:41  shark-admin
 *      comment added to MyParam::readParam()
 *
 *      Revision 2.6  2004/12/15 13:56:28  shark-admin
 *      switch for fitness functions
 *
 *      Revision 2.5  2004/12/14 17:13:26  shark-admin
 *      header and body formated
 *
 *      Revision 2.4  2004/11/15 16:24:00  shark-admin
...
int main (int argc, char* argv[])
{
  ...
  if ( param.Standard1 ){
    ...
    dynamic_cast< ChromosomeT< double >& >( parents[0][1] ).initializeGSA( param.SigmaMin, param.SigmaMax, param.baseSize );
  }
  else if ( param.IDA ){
    dynamic_cast< ChromosomeT< double >& >( parents[0][1] ).initializeIDA( param.SigmaMin, param.SigmaMax );
  }
  else if ( param.CMA ){
    dynamic_cast< ChromosomeT< double >& >( parents[0][1] ).initializeCMA( param.SigmaMin, param.SigmaMax );
  }
  else {
    exit(-1);
  }

...
int nOfPtsInterpolate = 3;    // number of points used for interpolation RWPL

float currBestFitness = 0;

int maxErrPercent = 10000;     // 100 times tolerance


//  to run :  foilminmax errpercent termination

int main(int argc, char **argv)
{
	if(argc>1) {
		maxErrPercent = atof(argv[1]);
		if(argc>2) term = atoi(argv[2]);
	}
...
// scaling so that we can handle negative objective scores.
	...
	ofstream bestPlot("bestPlot.dat", ios::trunc);
	bestPlot.close();
...
float foilObjective(GAGenome& c) {
	...
	float pdOut;
	//get f(x)     , the exact fitness of the nominal point
	...
	float lRange, uRange;
	float uncertainty;
	float ptsForInterpolate[nOfPtsInterpolate][2];  // 1st is uncertainty, 2nd is error fraction, e.g. 10%
	float worstPdOut;
	float previousPoint =0.0; 
	for(i=1; i<=nOfPtsInterpolate; i++) {
		uncertainty = (float)i*20.0;
		lRange = 0.5 - (0.5*uncertainty*0.1/100.0);
		uRange = 0.5 + (0.5*uncertainty*0.1/100.0);
		ptsForInterpolate[i-1][0] = uncertainty;
		if(currGeneration<nOfGenExact)
			worstPdOut = findWorstPdOutExact(arr, lRange, uRange, fx);
		else
			worstPdOut = findWorstPdOutApprox(arr, lRange, uRange, fx);
		worstPdOut = 100.0*(worstPdOut-fx)/fx;
	
		if(worstPdOut>previousPoint) {
			ptsForInterpolate[i-1][1] = worstPdOut;
			previousPoint = worstPdOut;
		}
		else ptsForInterpolate[i-1][1] = previousPoint;
		
	}

	ofstream trainFile("trainFile.dat", ios::trunc);
	trainFile<<0<<" "<<0<<endl;                 //add in one more point 0,0
	for(i=0; i<nOfPtsInterpolate; i++) {
		trainFile<<ptsForInterpolate[i][0]<<" "<<ptsForInterpolate[i][1]<<endl;
	}
	
	trainFile.close();

	float maxUncertainty = 1.0;
	float predictedError = 0.0;
	ofstream testFile;
	ifstream lwprPredict;
	while(predictedError<maxErrPercent) {
		testFile.open("testFile.dat", ios::trunc);
		testFile<<ptsForInterpolate[0][0]<<" "<<ptsForInterpolate[0][1]<<endl;
		testFile<<maxUncertainty<<" 0.0"<<endl;     //because we dont know yet, just put 0, no effect
		testFile.close();
		system("./lwprMod trainFile.dat testFile.dat 4");
		lwprPredict.open("lwprPredict.dat");
		lwprPredict>>predictedError;
		lwprPredict.close();
		//predictedError=100.0*(predictedError-fx)/fx;
		maxUncertainty=maxUncertainty+1.0;    // increase guess by 1%
	}

	//logging
	if(maxUncertainty>currBestFitness) {
		time_t currTime;
		time(&currTime);
	 	currBestFitness = maxUncertainty;
		ofstream bestPlot("bestPlot.dat", ios::app);
		bestPlot<<"Time: "<<asctime(localtime(&currTime))<<endl
			<<"New best chromosome found at generation "<<currGeneration<<endl
			<<"X = "<<endl;
		for(i=0; i<dimension; i++) {
			bestPlot<<arr[i]<<endl;
		}
		bestPlot<<"with maximum uncertainty of "<<maxUncertainty<<endl;
		bestPlot<<"uncertainty	error"<<endl;
		bestPlot<<"===========	====="<<endl;
		for(i=0; i<nOfPtsInterpolate; i++) {
			bestPlot<<ptsForInterpolate[i][0]<<"\t\t"<<ptsForInterpolate[i][1]<<endl;	
		}
		bestPlot<<endl<<endl;
		bestPlot.close();
	}

 ...
 *      $RCSfile: CrossEntropy.h,v $<BR>
 *      $Revision: 2.0 $<BR>
 *      $Date: 2003/11/28 16:23:22 $
 *
 *  \par Changes:
 *      $Log: CrossEntropy.h,v $
 ...
 *      stable
 */
class CrossEntropy : virtual public ModelInterface {
 ...
 */  
  double derror(const Array<double> &in, const Array<double> &target, bool returnError=true) {
    ...
    if (outputDimension > 1) {
      ...
      if(in.ndim() == 1) {
	// one input pattern
	//std::cout << "derror: Hallo x-Out 1-pattern! CE" << std::endl;
 	Array<double> output(target.dim(0));
	...
	for(unsigned c = 0; c < output.nelem(); c++){
	  ...
	  for(unsigned i = 0; i < dedw.nelem(); i++)
	    dedw(i) += (output(c)-target(c)) * dmdw(c, i);
	}
	if(isconsistent !=1){
	  std::cerr << std::endl << "Crossentropy: teacher labels do not sum to unity!" << std::endl;
	  exit(0);
	}
	...
	for(unsigned pattern = 0; pattern < in.dim(0); ++pattern) {
	  ...
	  for(unsigned c = 0; c < output.nelem(); c++){
	    ...
	    for(unsigned i = 0; i < dedw.nelem(); i++)
	      dedw(i) += (output(c)-target(pattern,c)) * dmdw(c, i);
	  }
	  if(isconsistent !=1){
	    std::cerr << std::endl << "Crossentropy: teacher labels do not sum to unity!" << std::endl;
	    exit(0);
	...
	for(unsigned i = 0; i < dedw.nelem(); i++)
	  if ((output(0)!=0) && (1. - output(0) != 0)){
	    dedw(i) -= (target(0) - output(0)) / (output(0) * (1. - output(0))) * dmdw(0, i);
	  } else { 
	    if (output(0) != target(0)){
	      // the sign changes due to the value of the target,
	      dedw(i) -= (2. * target(0) - 1.) * maxDeriv * dmdw(0, i);
	    }
	  }
      } else {
	...
	for(unsigned pattern = 0; pattern < in.dim(0); ++pattern) {
	  ...
	  for(unsigned i = 0; i < dedw.nelem(); i++)
	    if ((output(0)!=0) && (1. - output(0)!=0)){
	     dedw(i) -=(target(pattern, 0)-output(0)) / (output(0) * (1. - output(0))) * dmdw(0, i);
	    } else { 
		...
		dedw(i)-=(2. * target(pattern, 0) - 1.) * maxDeriv / in.dim(0) * dmdw(0, i);
	      }
	    }
	}
      }

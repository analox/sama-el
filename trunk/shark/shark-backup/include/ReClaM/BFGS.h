 ...
 *      INI Administration
 *
 *      Revision 1.3  2002/02/06 14:30:25  rudi
 ...
 *      stable
 */
class BFGS : virtual public ModelInterface {
 public:
   ...
   void bfgs     (const Array<double> &i, const Array<double> &t, 
		  double    gtol,
		  unsigned& iter,
		  double&   fret,
		  unsigned  iterMax);

   //! Initializes some internal variables used by the BFGS algorithm,
   //! calculates the current error of the model.
   void initBfgs (const Array<double> &in, const Array<double> &target, 
		  double&   fret, LineSearch ls = Dlinmin,
 ...
 */  
   void initBfgs (const Array<double> &in, const Array<double> &target, 
		  LineSearch ls = Dlinmin,
		  double ax = 0., double bx = 1., double lambda = 0.25) 
   {
       double fret;
       initBfgs(in, target, fret, ls, ax, bx, lambda);
   }
   ...
   void bfgs     (const Array<double> &in, const Array<double> &target, 
		  double& fret);
 ...
 *
 */  
   void setLambda (double x) { LS_lambda = x; }

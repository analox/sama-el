 ...
 *      $RCSfile: MixtureOfGaussians.h,v $<BR>
 *      $Revision: 2.0 $<BR>
			   ...
			   /*virtual*/ /*protected*/ public CodeBook
{
  ...
  public:
    ...
    double& mean ( unsigned i, unsigned j )       { return m( i, j ); }
    double& var  ( unsigned i, unsigned j )       { return v( i, j ); }
    double  mean ( unsigned i, unsigned j ) const { return m( i, j ); }
    double  var  ( unsigned i, unsigned j ) const { return v( i, j ); }

	//! Returns the array of mean vectors (read-only)
    const Array< double >& mean ( ) const         { return m;         }

	//! Returns the array of variance vectors (read-only)
    const Array< double >& var  ( ) const         { return v;         }

	//! Returns the array of mean vectors
    Array< double >& mean ( ) { return m; }

	//! Returns the array of variance vectors
    Array< double >& var  ( ) { return v;     }
    ...
    unsigned size( ) const { return MixtureModel< double >::size( ); }
    ...
    //
    bool operator < ( const MixtureOfGaussians& ) const { return false; }

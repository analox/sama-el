 ...
 *      $RCSfile: RBFN.cpp,v $<BR>
 *      $Revision: 2.1 $<BR>
 *      $Date: 2004/06/18 00:50:11 $
 *
...
void RBFN::setParams( const Array< double >& w )
{
	...
	{
        v.elem( i ) = std::max( sqr( w( k ) ), MIN_VAL );
	}
}
...
void RBFN::getParams( Array< double >& w ) const
{
    ...
    for( i = 0; i < v.nelem( ); ++i, ++k )
	{
        w( k ) = sqrt( std::max( v.elem( i ), MIN_VAL ) );
	}
}
    ...
    for( o = 0; o < odim( ); ++o )
	{
        ...
        for( k = 0; k < size( ); ++k )
		{
            ...
            for( i = 0; i < dim( ); ++i, ++dwi )
			{
                ...
                dw( o, dwi ) = Ae * t / std::max( s*s, MIN_VAL );
                dw( o, dwi + m.nelem( ) ) = Ae * t * t / std::max( s*s*s, MIN_VAL );
            }
        }
    }
}
...
void RBFN::initialize( const Array< double >& x, const Array< double >& y )
{
    ...
    //
    A.resize( A.dim( 0 ), MixtureOfGaussians::dim( ) );
    train_linear( x, y );
}
...
{
    A.resize( noutput, num );
    b.resize( noutput );
    MixtureOfGaussians::resize( num, ninput );
}

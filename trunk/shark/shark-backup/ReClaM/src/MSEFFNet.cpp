 ...
 *      INI Administration
 *
 *      Revision 1.5  2002/05/16 13:39:00  rudi
 *      doxygen commands added/modified.
 *
 *      Revision 1.4  2002/02/06 15:40:30  rudi
...
//===========================================================================
/*!
 *  \brief Method is not working yet. Will always exit with failure.
 *
 *  \param  input
 *  \param  output
 *  \return None.
 ...
 *
 */  
void MSEFFNet::dmodel(const Array<double> &input, Array<double> &output) {
  exit(EXIT_FAILURE);
};
 ...
 *
 */  
void MSEFFNet::dmodel  (const Array<double> &input) {
  exit(EXIT_FAILURE);
};
...
//===========================================================================
/*!
 *  \brief Calculates the mean squared error of the model output, compared to 
 *         the target values. 
 *
 ...
 *      Normalising now with no. of output neurons instead
 *      of no. of patterns.
 *
 *  \par Status
 *      stable
 *
 */  
double MSEFFNet::error(const Array<double> &input, const Array<double> &target) {
  double se = 0;
  if(input.ndim() == 1) {
 ...
 *  The results are stored in ModelInterface::dedw.
 ...
 *  \par Changes
 *      C. Igel, M. Toussaint, 2001-09-17:<BR>
 *      Normalising now with no. of output neurons instead of
 *      no. of patterns.
 *
 *  \par Status
 *      stable
 *
 */
double MSEFFNet::derror(const Array<double> &input, 
  ...
  dedw /= (double) target.nelem(); // mt & igel 20010917
  if (returnError)
 ...
 *
 */  
MSEFFNet::MSEFFNet(const string &filename) : FFNet(filename) { d.resize(numberOfNeurons); };

...
*
*  Copyright (c) 2004 Institut fuer Neuroinformatik
*
...
*      $RCSfile: pvmSphereGA.cpp,v $<BR>
*  
*    \par Changes:
*      $Log: pvmSphereGA.cpp,v $
*      Revision 2.1  2004/06/17 23:05:26  saviapbe
*      The standard file header was for doxygen adapted.
*
*      Revision 2.0  2004/05/25 13:40:20  saviapbe
*      Version tags are reset to 2.x
*  
*      Revision 1.1  2004/05/20 12:46:25  shark-admin
*      *** empty log message ***
*  
*  
*  <BR>
...
int main (int argn,char *argv[]){
  ...
  if(tid < 0){
    std::cerr << std::endl 
	      << "There is something going wrong ... " << std::endl
	      << "maybe you forgot to spawn processes from the pvm-console."  << std::endl << std::endl
	      << "Please try the following." << std::endl<< std::endl
	      << "Start the pvm-console, i.e., call the command '... /$(PVM-binpath)/pvm'." << std::endl
	      << "After the pvm-console has started, for example, add 3 hosts from your local network" << std::endl 
	      << "to your parallel virtual machine (PVM), i.e., call 'add host1 host2 host3'." << std::endl 
	      << "Start " << argv[0] << " from the pvm console, i.e, call"<< std::endl 
	      << "'spawn -> -4 $(SHARKHOME)/EALib/examples/" << argv[0] << " [optional random-seed]'." << std::endl << std::endl;
    exit(-1);
  } else 

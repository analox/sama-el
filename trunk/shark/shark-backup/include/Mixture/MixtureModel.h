...
class MixtureModel : public RandomVector< T >
{
  public:
	...
	if( x.ndim( ) == 2 ) {
		...
		for( i = 0; i < size( ); ++i )
		    s( i ) += pxi( i ) / std::max( px, /*MIN_VAL !!!*/ 1e-100 );
	    }
	    s /= double( x.dim( 0 ) );
	} else {
	    for( px = 0, i = 0; i < size( ); ++i )
    ...
    double& prior( unsigned i )           { return a( i ); }
    double  prior( unsigned i ) const     { return a( i ); }
    const Array< double >& prior( ) const { return a;      }

    unsigned size( ) const { return a.nelem( ); }

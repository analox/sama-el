 ...
 *      $RCSfile: MixtureOfGaussians.cpp,v $<BR>
 *      $Revision: 2.0 $<BR>
 *      $Date: 2003/11/28 16:23:12 $
 *
			       ...
			       Array< double >& z )
{
    ...
    if( x.ndim( ) == 2 ) {
        z.resize( x.dim( 0 ), x.dim( 1 ) + y.dim( 1 ) );
			      ...
			      const MixtureOfGaussians& old ) const
{
  ...
  */
	...
	pi /= std::max( px, MIN_VAL) ;
        for( i = size( ); i--; )
	    q += log( std::max( a( i ) * p( x[ k ], i ), MIN_VAL ) ) * pi( i );
    }
...
double MixtureOfGaussians::overlap( unsigned i, unsigned j ) const
{
	...
	marg -= sqr( m( j, k )-m( i, k ) ) / ( v( i, k )+v( j, k ) );
...
double MixtureOfGaussians::p( const Array< double >& x, unsigned i ) const
{
  //SIZE_CHECK( x.ndim( ) == 1 && x.nelem( ) <= dim( ) )
    ...
    for( unsigned j = x.dim( 0 ); j--; ) {
      //marg -= sqr( x.elem( j ) - m( i, j ) ) / std::max( v( i, j ), MIN_VAL );
	marg -= sqr( x.elem( j ) - m( i, j ) ) / v( i, j );
	//
...
double MixtureOfGaussians::Renyi( ) const
{
	...
	for( s = 1, k = 0; k < dim( ); ++k )
	    s *= v(i,k);
        sum += a(i) * a(i) / ( pow( 2*znSqrtPiC, int(dim( )) ) * sqrt( s ) );
    }

    for( i = 0; i < size( ); ++i ) {
        for( j = 0; j < i; ++j ) {
	    for( u = 0, s = 1, k = 0; k < dim( ); ++k ) {
	        u += sqr( m(i,k) - m(j,k) ) / ( v(i,k) + v(j,k) );
	        s *= v(i,k) + v(j,k);
            }
	    sum += a(i) * a(j) / ( pow( znSqrtPi2C, int(dim( )) ) * sqrt( s ) )
	         * exp( -u / 2 );
        }
    }
...
double MixtureOfGaussians::sqrDistance( const MixtureOfGaussians& mix ) const
{
  //SIZE_CHECK( dim( ) == mix.dim( ) )
    ...
    for( i = 0; i < size( ); ++i )
		...
		uij += sqr( m(i,k) - m(j,k) ) / ( v(i,k) + v(j,k) );
	        sij *= v(i,k) + v(j,k);
	    }
	    sum += a(i) * a(j) * exp( -uij / 2 )
	         / ( pow( znSqrtPi2C, int(dim( )) ) * sqrt( sij ) );
	}

    for( i = 0; i < size( ); ++i )
        for( j = 0; j < mix.size( ); ++j ) {
	    for( uij = 0, sij = 1, k = 0; k < dim( ); ++k ) {
		uij += sqr( m(i,k) - mix.m(j,k) ) / ( v(i,k) + mix.v(j,k) );
	        sij *= v(i,k) + mix.v(j,k);
	    }
...
)
{
	...
	for( j = dim( ); j--; ) {
	    ...
	    v( i, j ) += pi( i ) * ( sqr( x( j ) - m( i, j ) ) - v( i, j ) );
	}
    }
}
...
)
{
  ...
  //SIZE_CHECK( x.ndim( ) == 1 )
    ...
    for( unsigned i = 0; i < x.nelem( ); ++i )
        if( x.elem( i ) < min( bound1.elem( i ), bound2.elem( i ) ) ||
	    x.elem( i ) > max( bound1.elem( i ), bound2.elem( i ) ) )
	    return false;
...
{
    static char *p, fmt[ 30 ];

    sprintf( fmt, "%1.16g", x );

    if( ( p = strchr( fmt, 'e' ) ) != NULL ) {
        *p++ = '\0';
	os << fmt << " 10^(" << p << ')';
    } else
        os << fmt;
  ...
  //SIZE_CHECK( x.ndim( ) == 2 )
    ...
    if( size( ) == 1 )
	{
        ...
        for( i = dim( ); i--; )
		{
            ...
            v( 0, i ) = newV( i ) / x.dim( 0 ) - sqr( m( 0, i ) );
        }
		return true;
    }
	else
				...
				for( i = size( ); i--; )
				{
					...
					for( j = dim( ); j--; )
					{
						if( newA( i ) > MIN_VAL )
						{
							...
							v( i, j ) = std::max( newV( i, j ) / newA( i )
									  - sqr( m( i, j ) ), minVariance );
						}
					}
				}
			}
	...
	for( iteration = 0; iteration < maxIteration; ++iteration ) {
		...
		px = std::max( px, MIN_VAL );
		    ...
		    diffa      *= x.dim( 0 ) / ( x.dim( 0 ) - 1. );
		        ...
		        jackV( i, j ) -= ( newV( i, j ) - pi( i ) * sqr( x( k, j ) ) ) / diffa;
                    }
                }
	    }
		...
		for( j = dim( ); j--; ) {
		    m( i, j ) = jackM( i, j );
		    v( i, j ) = std::max( jackV( i, j ) - sqr( m( i, j ) ),
					  minVariance );
		}
	    }
				       ...
				       unsigned i, double maxV ) const
{
  ...
  */
    ...
    return exp( marg / 2 ) / sqrt( std::max( varg, MIN_VAL ) * pow( 2 * znPiC, int(d) ) );
}
						   ...
						   double maxV ) const
{
  //SIZE_CHECK( x.ndim( ) == 2 )
    ...
    for( unsigned k = x.dim( 0 ); k--; )
        l += log( std::max( deficientP( x[ k ], xvar[ k ], maxV ), 1e-100 ) );  // !!!
...
double MixtureOfGaussians::E( unsigned i, unsigned j ) const
{
    ...
    return exp(-u/2) / ( pow( znSqrtPi2C, int(dim()) ) * sqrt(s) );
}

double MixtureOfGaussians::U( unsigned i, unsigned j, unsigned r ) const
{
    return ( 3 * sqr( v(i,r) + v(j,r) )
             - 6 * ( v(i,r) + v(j,r) ) * sqr( m(i,r) - m(j,r) )
             + pow( m(i,r) - m(j,r), 4 ) ) /
           pow( v(i,r) + v(j,r), 4 );
}
...
{
    return ( v(i,r) + v(j,r) - sqr( m(i,r) - m(j,r) ) )
         * ( v(i,t) + v(j,t) - sqr( m(i,t) - m(j,t) ) ) /
           ( sqr( v(i,r) + v(j,r) ) * sqr( v(i,t) + v(j,t) ) );
}
...
{
    return ( 12*(v(j,q)+v(s,q)) - 4*sqr(m(j,q)-m(s,q)) )
	      / pow( v(j,q) + v(s,q), 3 );
}

double MixtureOfGaussians::Delta2( unsigned s, unsigned j, unsigned q ) const
{
  return (36*(v(j,q)+v(s,q))*sqr(m(j,q)-m(s,q))
		 -12*sqr(v(j,q)+v(s,q))
		 -8*pow(m(j,q)-m(s,q),4))
	      / pow(v(j,q)+v(s,q),4);
}
...
{
    return 2 * (v(j,k)+v(s,k)-sqr(m(j,k)-m(s,k)))
      / ((v(j,q)+v(s,q))*sqr(v(j,k)+v(s,k)));
}
				   ...
				   unsigned q, unsigned k ) const
{
    return Delta3( s, j, q, k )
      * (2*sqr(m(j,q)-m(s,q))/(v(j,q)+v(s,q))-1);
}
...
double MixtureOfGaussians::dAds( unsigned r, unsigned s, unsigned q ) const
{
    ...
    for( unsigned j = 0; j < size( ); ++j )
	  ...
	  * ( U( s, j, r ) * ( sqr( m(j,q)-m(s,q) ) / (v(j,q)+v(s,q)) -1 )
				 ...
				 unsigned s, unsigned q ) const
{
    ...
    for( unsigned j = 0; j < size( ); ++j )
        sum += a( j ) * E( s, j )
	  * ( sqrt(v(s,q)) / ( v(j,q)+v(s,q) ) )
	  * ( V( s, j, r, t ) * ( sqr( m(j,q)-m(s,q) ) / (v(j,q)+v(s,q)) -1 )
...
Array< double > MixtureOfGaussians::overallVariance( ) const
{
	...
	if( size( ) > 0 )
	{
		...
		variance = a( 0 ) * ( v[ 0 ] + sqr( m[ 0 ] - mean ) );
		for( unsigned i = size( ) - 1; i--; )
		{
			variance += a( i ) * ( v[ i ] + sqr( m[ i ] - mean ) );
		}
	}

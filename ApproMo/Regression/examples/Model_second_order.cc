...
int Model_second_order(Array<double> InputData,
  ...
  for (int i = 1; i <= N; i++) { 
    ...
    for (int pp = 1; pp <= D; pp++) {
      a[i][pp+D+1] = InputData(start+i-1,pp-1) * InputData(start+i-1,pp-1);
    }
    
    b[i] = TargetData(i-1,0);
  }
...
int Model_second_order_interaction(Array<double> InputData,
  ...
  for (int i = 1; i <= N; i++) { 
    ...
    if (interaction >= 1) {
      int qq = 1; 
      int rr = qq+1;
      for (int pp = 1; pp <= interaction; pp++) {
	a[i][pp+2*D+1] = InputData(start+i-1,qq-1) * InputData(start+i-1,rr-1);
        if (rr == D) {
	  qq++;
	  rr = qq+1;
	}
